...
const PatchMemoryHandler = module.exports = function PatchMemoryHandler () {
}
 ...
 */
PatchMemoryHandler.prototype.ready = false
 ...
 */
PatchMemoryHandler.prototype.initialise = function (resourceConfig) {
 ...
 */
PatchMemoryHandler.prototype.search = function (request, callback) {
  const self = this
  ...
  return callback(null, PatchMemoryHandler._clone(results), resultCount)
}

/**
  Find a specific resource, given a resource type and and id.
 */
PatchMemoryHandler.prototype.find = (request, callback) => {
  ...
  return callback(null, PatchMemoryHandler._clone(theResource))
}

/**
  Create (store) a new resource given a resource type and an object.
 */
PatchMemoryHandler.prototype.create = (request, newResource, callback) => {
  // Check to see if the ID already exists
  const index = PatchMemoryHandler._indexOf(resources[request.params.type], newResource)
  if (index !== -1) {
    return callback({ // eslint-disable-line standard/no-callback-literal
      status: '403',
      code: 'EFORBIDDEN',
  ...
  return callback(null, PatchMemoryHandler._clone(newResource))
}

/**
  Delete a resource, given a resource type and and id.
 */
PatchMemoryHandler.prototype.delete = function (request, callback) {
  // Find the requested resource
  this.find(request, (err, theResource) => {
    if (err) return callback(err)

    // Remove the resource from the in-memory store.
    const index = PatchMemoryHandler._indexOf(resources[request.params.type], theResource)
 ...
 */
PatchMemoryHandler.prototype.update = function (request, partialResource, callback) {
  // Find the requested resource
  this.find(request, (err, theResource) => {
    if (err) return callback(err)
 ...
 */
PatchMemoryHandler.prototype._sortList = (request, list) => {
...
PatchMemoryHandler._clone = obj => JSON.parse(JSON.stringify(obj))

PatchMemoryHandler._indexOf = (list, obj) => {
  for (const i in list) {
